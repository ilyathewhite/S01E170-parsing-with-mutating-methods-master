...
//  ParseCSV
//
//  Created by Chris Eidhof on 17.11.17.
...
try time {
    ...
    let file = URL(fileURLWithPath: "/Users/chris/Downloads/worldcitiespop.txt")
    ...
    while !remainder.isEmpty {
        try parseLine(&remainder, result: &x, processField: { state, _, field in
            ()
        })
        if result < 10 {
            print("result: \(result)")
